...
c = datetime.datetime(100, 1, 1, 0,0,0)
for i in range(number_of_cars):
    if data[i][4] == "F":
        a = datetime.datetime(100, 1, 1, int(data[i][0]), int(data[i][1]), int(data[i][2]))
        b = a + datetime.timedelta(0, int(data[i][3]))
        if c > b:
            txt_w.writelines(f'{str(c.time()).split(":")[0]} {str(c.time()).split(":")[1]} {str(c.time()).split(":")[2]}\n')

        else:
            txt_w.writelines(f'{str(b.time()).split(":")[0]} {str(b.time()).split(":")[1]} {str(b.time()).split(":")[2]}\n')
        c = a + datetime.timedelta(0, int(data[i][3]))
